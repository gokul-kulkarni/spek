...
- [ ] 8.2 Build the VS Code webview (`npm run build:webview`), package and run it, and verify a diagram shows its source with no error state and no CSP violation in the developer console. The drawing path is not exercised here — this build does not draw (design D2).
