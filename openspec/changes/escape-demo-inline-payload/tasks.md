...
- [ ] 3.4 Verify the action by hand, as CLAUDE.md requires for changes to the generated HTML's content or to input handling: on the pull request branch, add a temporary workflow that runs `uses: ./` with `spek-version: ${{ github.sha }}` and `title: '</title><b>R&amp;D'`, and asserts that the output's `<title>` line reads `<title>&lt;/title&gt;&lt;b&gt;R&amp;amp;D</title>` and that the file holds exactly two `</script>` (case-insensitive). The branch's own payload carries this change's artifacts, so the same run exercises the payload escape on real content. Confirm the run is green, then delete the workflow before merge.
